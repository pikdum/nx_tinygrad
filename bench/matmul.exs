# f32 matmul benchmark: BinaryBackend vs nx_tinygrad (compile vs warm replay).
#
#   mix run bench/matmul.exs
#   NX_TINYGRAD_BENCH_DEVICE="KFD+AMD:LLVM" mix run bench/matmul.exs   # on GPU
...
worker =
  if device == "CPU" do
    ...
    {:ok, _} = NxTinygrad.WorkerSupervisor.start_worker(:bench, device)
    :bench
  end
...
# nx_tinygrad, device-resident inputs and outputs
adev = Nx.backend_transfer(a, {Backend, worker: worker})
bdev = Nx.backend_transfer(b, {Backend, worker: worker})
mm = NxTinygrad.jit(fn x, y -> Nx.dot(x, y) end, worker: worker, output: :device)

{compile_us, _} = :timer.tc(fn -> mm.(adev, bdev) end)
IO.puts("nx_tinygrad compile  : #{Float.round(compile_us / 1000.0, 2)} ms (first call)")

warm_ms = avg.(fn -> mm.(adev, bdev) end, 20)
IO.puts("nx_tinygrad warm     : #{Float.round(warm_ms, 3)} ms/call (resident in+out, replay)")
