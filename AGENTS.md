...
Guidance for coding agents working on `ex_tinygrad` — an Elixir `Nx` compiler and
...
- The dev shell provides Elixir 1.20 / OTP 29, Rust, and a ROCm-free tinygrad
  Python (`EX_TINYGRAD_PYTHON`). `MIX_REBAR3` points at a working rebar3 (the
  OTP-29 build is broken upstream).
- Toolchain is pinned via `flake.lock` (nixos-unstable). See `STATUS.md` for versions.

## Structure

- `lib/ex_tinygrad/` — Elixir: `compiler.ex` (`Nx.Defn.Compiler`), `lowering.ex`
...
- `priv/worker/` — Python worker: `main.py` (loop/dispatch), `protocol.py`,
  ...
  `device.py`, `dtype.py`.
- `native/ex_tinygrad_ref/` — Rust NIF: GC-triggered buffer release queue.
...
- GPU tests (needs an AMD device): `EX_TINYGRAD_GPU_TESTS=1 mix test test/gpu`
- Python worker tests: `python -m pytest -q worker_tests`
- Format: `mix format`
- Benchmarks: `EX_TINYGRAD_GPU_TESTS=1 mix run bench/nx_backends.exs`
...
- **Never silently fall back to another backend** inside a compiled graph —
  unsupported ops raise `ExTinygrad.CompileError` at compile time.
- Keep the Elixir and Python sides of the **dtype** and **device** mappings in
  sync (`lib/ex_tinygrad/dtype.ex` ↔ `priv/worker/dtype.py`, `device.ex` ↔
  `device.py`).
- Every external handle carries a worker **generation**; a stale handle raises
  `ExTinygrad.StaleTensorError`, never wrong data.
...
- If the change touches the GPU/AMD path, run `EX_TINYGRAD_GPU_TESTS=1 mix test test/gpu`.
