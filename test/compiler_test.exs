defmodule ExTinygrad.CompilerTest do
  ...
  setup do
    assert ExTinygrad.Worker.whereis(:default) != nil
    :ok
  end
  ...
  test "elementwise chain" do
    ...
    assert_close(ExTinygrad.jit(&G.elementwise/2).(x, y), G.elementwise(x, y))
  end

  test "broadcasting" do
    x = Nx.iota({2, 3}, type: :f32)
    b = Nx.tensor([10.0, 20.0, 30.0])
    assert_close(ExTinygrad.jit(&G.broadcasting/2).(x, b), G.broadcasting(x, b))
  end

  test "reduction (with and without keep_axes)" do
    x = Nx.iota({3, 4}, type: :f32)
    assert_close(ExTinygrad.jit(&G.reduction/1).(x), G.reduction(x))
    assert_close(ExTinygrad.jit(&G.reduce_keep/1).(x), G.reduce_keep(x))
  end
  ...
  test "matrix multiplication" do
    ...
    assert_close(ExTinygrad.jit(&G.matmul/3).(x, w, b), G.matmul(x, w, b))
  end

  test "comparison + select" do
    x = Nx.tensor([[1.0, 5.0], [3.0, 2.0]])
    y = Nx.tensor([[4.0, 4.0], [4.0, 4.0]])
    assert_close(ExTinygrad.jit(&G.comparison/2).(x, y), G.comparison(x, y))
  end

  test "shape ops (transpose/reshape)" do
    x = Nx.iota({2, 3}, type: :f32)
    assert_close(ExTinygrad.jit(&G.shapes/1).(x), G.shapes(x))
  end

  test "softmax (reduce_max, exp, sub, sum, divide)" do
    x = Nx.tensor([[1.0, 2.0, 3.0], [1.0, 1.0, 1.0]])
    assert_close(ExTinygrad.jit(&G.softmax/1).(x), G.softmax(x))
  end

  test "multiple outputs (tuple)" do
    x = Nx.iota({2, 3}, type: :f32)
    {a, b} = ExTinygrad.jit(&G.multi_output/1).(x)
  ...
  test "nested output container (map + tuple + list)" do
    ...
    got = ExTinygrad.jit(&G.nested_container/2).(x, y)
  ...
  test "compiling twice yields equal results (idempotent)" do
    ...
    r2 = ExTinygrad.jit(&G.reduction/1).(x)
    assert_close(r1, r2)
  end
end
