defmodule ExTinygrad.OutputContainerTest do
  @moduledoc "Container flattening and reconstruction, exercised end-to-end."
  use ExUnit.Case, async: false

  import Nx.Defn
  import ExTinygrad.TestGraphs, only: [assert_close: 2]
  ...
  test "single tensor" do
    assert_close(ExTinygrad.jit(&single/1).(@x), single(@x))
  end

  test "tuple" do
    {a, b} = ExTinygrad.jit(&tuple2/1).(@x)
    {ea, eb} = tuple2(@x)
    assert_close(a, ea)
    assert_close(b, eb)
  end
  ...
  test "nested tuple" do
    {a, {b, c}} = ExTinygrad.jit(&nested/1).(@x)
    {ea, {eb, ec}} = nested(@x)
    assert_close(a, ea)
    assert_close(b, eb)
    assert_close(c, ec)
  ...
  test "empty tuple" do
    assert ExTinygrad.jit(&empty_tuple/1).(@x) == {}
  end

  test "repeated output expression" do
    {a, b} = ExTinygrad.jit(&repeated/1).(@x)
  ...
  test "struct implementing Nx.Container (Nx.Batch-like via Complex? use a tuple-of-map)" do
    ...
    got = ExTinygrad.jit(fn x -> %{outer: {Nx.negate(x)}, flat: Nx.add(x, 2.0)} end).(@x)
