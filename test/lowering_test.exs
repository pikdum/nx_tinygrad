defmodule ExTinygrad.LoweringTest do
  use ExUnit.Case, async: true

  alias ExTinygrad.{Graph, GraphCacheKey, Lowering}
  ...
  test "matmul lowers to dot/add/tanh with three inputs and one output" do
    ...
    b = Nx.tensor([1.0, 2.0])
    graph = lower(&ExTinygrad.TestGraphs.matmul/3, [x, w, b])
  ...
  test "ids are sequential and topological (children before parents)" do
    graph = lower(&ExTinygrad.TestGraphs.chain/1, [Nx.iota({3}, type: :f32)])
  ...
  test "comparison produces a u8 node feeding select" do
    ...
    graph = lower(&ExTinygrad.TestGraphs.comparison/2, [a, a])
  ...
  test "reduction normalizes axes and records keep_axes" do
    graph = lower(&ExTinygrad.TestGraphs.reduce_keep/1, [Nx.iota({2, 3}, type: :f32)])
  ...
  test "a repeated subexpression is emitted once (DAG dedup)" do
    graph = lower(&ExTinygrad.TestGraphs.repeated_input/1, [Nx.iota({4}, type: :f32)])
  ...
  test "multiple outputs are preserved in order" do
    graph = lower(&ExTinygrad.TestGraphs.multi_output/1, [Nx.iota({2, 3}, type: :f32)])
  ...
  test "canonical JSON and cache key are deterministic for identical graphs" do
    ...
    w = Nx.iota({3, 2}, type: :f32)
    b = Nx.tensor([1.0, 2.0])
    g1 = lower(&ExTinygrad.TestGraphs.matmul/3, [x, w, b])
    g2 = lower(&ExTinygrad.TestGraphs.matmul/3, [x, w, b])
  ...
  test "different input shapes produce different cache keys" do
    b = Nx.tensor([1.0, 2.0])
    g1 = lower(&ExTinygrad.TestGraphs.matmul/3, [Nx.iota({2, 3}, type: :f32), Nx.iota({3, 2}, type: :f32), b])
    g2 = lower(&ExTinygrad.TestGraphs.matmul/3, [Nx.iota({4, 3}, type: :f32), Nx.iota({3, 2}, type: :f32), b])
  ...
  test "unsupported operations raise a compile error before Python" do
    assert_raise ExTinygrad.CompileError, ~r/unsupported Nx operation/, fn ->
