defmodule NxTinygrad.GradientTest do
  @moduledoc """
  ...
  test "linear+tanh value_and_grad matches BinaryBackend" do
    ...
    {value, grad} = NxTinygrad.jit(&G.linear_value_and_grad/3).(w, x, targets)
  ...
  test "MLP inference matches BinaryBackend" do
    p = params()
    {x, _y} = inputs()
    assert_close(NxTinygrad.jit(&G.mlp_predict/2).(p, x), G.mlp_predict(p, x), @tol)
  end
  ...
  test "MLP training step: value_and_grad over all parameters matches BinaryBackend" do
    ...
    {value, grads} = NxTinygrad.jit(&G.mlp_value_and_grad/3).(p, x, y)
  ...
  test "a gradient-descent step reduces the loss" do
    ...
    vg = NxTinygrad.jit(&G.mlp_value_and_grad/3)
